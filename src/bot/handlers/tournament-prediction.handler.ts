...
interface TournamentPredictionSession {
  ...
  first?: string;
  second?: string;
  modifyingSinglePosition?: 'first' | 'second'; // Track if modifying single position
}
  ...
  try {
    ...
    if (existing) {
      ...
      if (existing.is_scored) {
        message += `✅ FINAL SCORE\n`;
        message += `━━━━━━━━━━━━━━━━━━━━\n`;
        message += `${existing.bonus_points}/14 bonus points earned! 🎉\n`;
      } else {
        message += `⏳ PENDING\n`;
        message += `━━━━━━━━━━━━━━━━━━━━\n`;
        message += `7 points for each correct position\n`;
        message += `Maximum: 14 bonus points\n\n`;
...
export async function handleTeamSelection(ctx: Context): Promise<void> {
  try {
    ...
    const position = parts[2] as 'first' | 'second';
...
export async function handleTournamentPredictionConfirm(ctx: Context): Promise<void> {
  try {
    ...
    const callbackData = ctx.callbackQuery.data;
    const parts = callbackData.split('_'); // tp_confirm_first_second
    const first = parts[2];
...
export async function handleTournamentPredictionModify(ctx: Context): Promise<void> {
  try {
    ...
    if (callbackData === 'tp_modify_all') {
      ...
      const positionMap: Record<string, 'first' | 'second'> = {
      ...
      const positionLabels = { first: '1st', second: '2nd' };
      const positionEmojis = { first: '🥇', second: '🥈' };
